...
export class StockService {
    ...
    public STOCK_KEY: string = 'stocks';
    public SUSTAIN_KEY: string = 'sustainability'
    public LEADERBOARD_KEY: string = 'leaderboard'
    public user_stocks: UserStock[] = [];
    public user_spending: number = 100000;
    public user_sus: string = 'N/A';
    public user_port: number = 0;
    public stockData: StockDictionary = {}
    ...
    public addStockToPortfolio(stock: string, quantity: number): string {
        ...
        this.user_stocks.push({ ticker: stock_to_add.ticker, quantity: quantity, price: stock_to_add.price, industry: stock_to_add.industry });
        return JSON.stringify(this.user_stocks);
    }
    ...
    public getFinalScore(): number {
        ...
        result = result * 100;
        // Diversity penalty
        if (!this.isDiverse) { result -= 5 };

        return Math.round(result);
    }

    // code to run between weeks
    private updateWeek(): string {
        if (this.WEEK < 4) {
            this.updatePortfolio;
            return this.getUserStocks();
        }
        else {
            // [final score, sustainability score, portfolio value]
            return JSON.stringify([this.getFinalScore(), this.user_sus, this.user_port]);
        }
    }

    private isDiverse() {
        let industries = new Set<string>;
        this.user_stocks.forEach(stock => { industries.add(stock.industry) });
